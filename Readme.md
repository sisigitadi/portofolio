...
- **No Tracking, No Cookies**: The site ships no analytics, no advertising, and no tracking scripts, and sets no cookies. Browser `localStorage` is used only for device-local preferences (theme, language, submit-rate timestamp) and never contains personal data. *Optional* visitor tracker (`worker-visitor/`): a self-hosted Cloudflare Worker hit counter + private owner dashboard — disabled by default (zero outbound requests until `WORKER_URL` is set in `index.html`), stores only salted SHA-256 IP hashes (raw IPs are never persisted, UU PDP friendly) and reads geolocation from the Cloudflare edge.
- **Third-Party Data Flows (complete list)**: The only outbound network call is the contact form → Formspree (`https://formspree.io/f/mkgknrqk`). Project preview images are served from local `assets/` (no third-party image CDN). When the optional visitor tracker is enabled (see `worker-visitor/README.md`), the page additionally sends one anonymous page-view ping to **your own** Cloudflare Worker (`POST /hit`) and polls `GET /count` every 60s — no third-party analytics involved.
...
> **Arsip eksplorasi desain — terpisah dari situs live.** **Status v2.6.1: Field Manual sudah diimplementasikan sebagai `index.html` produksi** (satu file tanpa build step, lolos gate: audit 13 PASS · pytest 62/62 · Lighthouse a11y/BP/SEO 100). Galeri `design-previews/` dan `design-lab/` di bawah ini tetap sebagai arsip referensi desain di luar gerbang audit — perubahan desain berikutnya disalurkan ke `index.html` melewati gate yang sama.

- **`design-previews/` — 5 arah desain HTML statis** (buka `design-previews/index.html` sebagai galeri): `01-soc-console` (dark, amber phosphor, log lines), `02-field-manual` (**konsep terpilih** — kertas krem/tinta/aksen karat, dokumen manual cetak), `03-trade-journal` (editorial print), `04-signal-monitor` (panel instrumen), `05-plaintext-brutalist` (man-page, nol gradien). Semua dibangun dari konten nyata portfolio, tanpa runtime styling CDN, tanpa look template AI.
- **Field Manual (`02-field-manual.html`) — konsep terpilih, diimplementasikan ke produksi v2.6.1**: konten 100% akurat terhadap CV (spec table, 4 proyek + URL produksi, field log 2002–2026, 9 sertifikasi, field reports), TOC sebagai satu-satunya navigasi, lembar kertas di atas meja, skip-link a11y, dan **Request Slip — Resume (PDF)** — form satu-field (email) yang **terhubung ke endpoint Formspree produksi** (`https://formspree.io/f/mkgknrqk`, honeypot + throttle 30 dtk, hidden `source: field-manual`).
- **Coretan tinta "manual yang dibaca recruiter"**: 16 mark (centang/bintang/paraf/catatan margin) menggambar saat scroll via `stroke-dashoffset` + IntersectionObserver, font tulisan tangan **Caveat** (Google Fonts) + jitter per-huruf — semua `aria-hidden`, mati di `prefers-reduced-motion`. **Saat implementasi ke produksi**: tambahkan `family=Caveat:wght@500;600` ke stylesheet Google Fonts yang sudah ada (bukan link baru).
- **`design-lab/` — workshop konsep 3D (React + Vite + @react-three/fiber + drei + framer-motion)**: 10 konsep tema futuristik (radar dome, neural space, mission deck, container registry, identity vault, career orbit, forensics files, syslog vault, ATC tower, promptable console) — dokumentasi lengkap di `design-lab/CONCEPTS.md`.
