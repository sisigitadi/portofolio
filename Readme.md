...
- **🛰️ Visitor Tracker (active)**: hit-counter badge in the footer plus a private owner-only dashboard (IP hash, city/country/timezone, referrer, page, user-agent) powered by `worker-visitor/` (Cloudflare Worker + D1). The dashboard ships charts (30-day trend, hourly), top countries/cities, device/browser/OS breakdown, a world dot map, CSV export, path filter, pagination and 60s auto-refresh — all client-side, no CDN. No cookies; raw IPs never stored. Owner access: click the footer copyright text **9×** to open the dashboard (key never embedded in the site; the login page can remember it in the owner's browser for auto-unlock).
