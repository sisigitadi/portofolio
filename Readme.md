...
- **No Runtime Styling CDN**: CSS is split into two layers: `styles.css` (347 lines — shared variables, layout, typography, dark mode, responsive breakpoints) loaded via `<link rel="stylesheet">`, and a minimal inline `<style>` block (~390 lines) containing only `@media print` rules (required for ATS audit compliance). All 3 HTML files share the same external stylesheet. No Tailwind CDN, no build step. Remaining third-party assets are limited to CDN-hosted fonts/icons (Google Fonts, Font Awesome via cdnjs) and external services (Formspree form delivery); project preview images are served from local `assets/`.
...
- **Pre-Flight Audit & Git Gate**: `audit.py` runs **15 registered checks** (form endpoint, external-link safety, relative paths, WCAG gimmick isolation, HTML tag balance, inline-script syntax via `node --check`, testimonial sync, i18n parity, `getElementById`/`querySelector`/`closest`/`matches` DOM resolution, duplicate-id detection, **SEO meta** — title ≤ 65 / description ≤ 160 / robots `index` / canonical / OG / Twitter — **JSON-LD structured-data validity** — `Person` + `WebSite` required — **ATS Print Mode verification** for single-column A4 typography — and **CSP script-src hash ↔ inline script sync**). Each check is a registered method (`@register`). Modes: `python audit.py` (full), `python audit.py --quick` (skips `node --check` for fast pre-commit feedback), and `python audit.py path/to/file.html` (audit any target; default `index.html`). Every run ends with a **summary line**: `Summary: 15 PASS | 0 FAIL | 0 WARN | 15 checks | 0.3s`.
- **Unit tests (`test_audit.py`, `test_indexnow_ping.py`, pytest)**: 72 tests lock in the audit and indexnow behavior — valid pages pass, every targeted breakage is caught with exactly one FAIL, single-language pages pass feature checks cleanly, and CLI arg parsing is parametrized. Run with `python -m pytest test_audit.py test_indexnow_ping.py -v`.
- **Four gates, defense in depth**: (1) `.githooks/pre-commit` runs the audit in `--quick` mode for early detection; (2) `.githooks/pre-push` runs the full audit and **blocks the push** on any failure; (3) `.github/workflows/preflight.yml` runs the full audit **plus the full pytest suite (72 tests: `test_audit.py` + `test_indexnow_ping.py`) plus the visitor-worker route tests** on every push/PR to `main` as a CI gate; (4) `.github/workflows/lighthouse-ci.yml` runs **Lighthouse CI** on every push/PR — **accessibility, best-practices & SEO must be 100** (error, blocks push/PR), performance ≥ 50 (warn, non-blocking — synthetic mobile-throttle score is noisy). Config in `.lighthouserc.json`.
...
- **🌗 System-Preference Dark Mode**: Automatic dark/light theme via `@media (prefers-color-scheme: dark)` CSS media query — no toggle needed, follows OS setting. Dark palette: navy background, warm white text, bright rust accent. Dynamic `<meta name="theme-color">` syncs mobile status bar. Print stylesheet unaffected.
...
- **Field Manual (`02-field-manual.html`) — chosen concept, implemented into production v2.6.1**: 100% CV-accurate content (spec table, 4 projects + production URLs, career history, 9 certifications, testimony), item-numbered sections (corner badges `1.01`–`5.09`), TOC as the only navigation, paper sheet on a desk, a11y skip-link, and **Request Resume (PDF)** — a single-field (email) form **connected to the production Formspree endpoint** (`https://formspree.io/f/mkgknrqk`, honeypot + 30s throttle, hidden `source: field-manual`).
...
- **Content Security Policy (meta tag) — hardened (v2.6.1)**: The page ships a strict CSP that **blocks all external scripts and any inline script not covered by an exact `sha256-…` hash** (`script-src 'self' 'sha256-…' 'sha256-…'` — no third-party JS, no `eval`, no `'unsafe-inline'` for scripts); allows first-party + inline styles plus Google Fonts / Font Awesome (cdnjs); fonts from Google / cdnjs / `data:`; images restricted to `'self'`, `data:` and the visitor-tracker Worker; and **network connections restricted to Formspree + the visitor-tracker Worker only** (`connect-src 'self' https://formspree.io https://portofolio-visitor-tracker.si-sigitadi.workers.dev` — no wildcard `https:` fallback, so data cannot be exfiltrated to unknown origins). Additional directives: `object-src 'none'`, `frame-src 'none'`, `base-uri 'self'`, `form-action 'self' https://formspree.io`, `report-uri` (CSP violation monitoring to self-hosted worker), `upgrade-insecure-requests`, plus a **`Permissions-Policy`** disabling camera/mic/geolocation/payment/usb/autoplay. **CSP Violation Monitoring**: the `report-uri` directive sends CSP violations to the self-hosted worker (`/csp-report`), storing them in D1 for owner review — stale hashes, injection attempts, and unexpected external resources are logged without third-party services. **Maintenance (important)**: any change to an inline `<script>` invalidates its hash — recompute it and update the CSP (Project_rules §1.7), otherwise the script is silently blocked and the page breaks (the new audit check #14 now detects this automatically).
