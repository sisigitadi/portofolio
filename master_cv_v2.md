# SIGIT ADI IRIANTO
**IT Infrastructure, SecOps, and Applied AI Engineer**  
Banten, Indonesia (UTC+7) | Open to Worldwide Remote Work  
Website: sigitadi.id | Email: x@sigitadi.id | LinkedIn: linkedin.com/in/sigitadi | GitHub: github.com/sisigitadi

---

## PROFESSIONAL SUMMARY
24+ years in IT infrastructure, systems operations, and technology delivery, with recent specialization in Security Operations and Applied AI engineering. Proven track record of bridging enterprise systems administration with automated workflows, including Linux server hardening, Wazuh SIEM threat triage, FortiWeb WAF alert analysis, and LLM evaluation suites. Demonstrated capability in building publicly deployed applications, optimizing SQL database workflows, reducing operational response times, and deploying containerized architectures across private sector and government environments.

---

## TECHNICAL SKILLS

*   **Enterprise IT Infrastructure & Systems:** Linux Server Administration, Linux Hardening, Windows Server, Docker Containerization, WSL, Systems Administration, Local Network (LAN/WAN), Backup & Disaster Recovery, SQL Database Administration.
*   **Security Operations (SecOps) & Threat Defense:** Wazuh SIEM Configuration & Agent Telemetry, Incident Response Playbooks (NIST SP 800-61), Tier-1 SOC Security Monitoring, Threat Hunting, FortiWeb WAF Alert Triage, Vulnerability Assessment, Server Baseline Reviews.
*   **Applied AI & LLM Engineering:** Gemini API Integration, LangChain, Ollama Local Inference, Prompt Stability Testing, LLM Evaluation & Benchmarking, Structured Output Validation (JSON Schema), n8n AI Automation.
*   **Software Engineering & Workflow Automation:** Python, REST API Integration, TypeScript, Next.js, Streamlit, n8n Workflows, Git & GitHub, JSON/XML Data Pipelines.

---

## TECHNICAL PROJECTS

### **PromptMatrix - LLM Evaluation & Prompt Stability Platform**
*Stack:* TypeScript, Next.js, LangChain, Gemini API, JSON Schema Validation | *Live:* https://sisigitadi.github.io/promptmatrix2 | *Source:* https://github.com/sisigitadi
*   Architected a client-side prompt evaluation engine testing LLM prompt stability, format adherence, and safety across systematic prompt variations.
*   Quantified prompt variant stability, drift risks, and strict JSON schema format adherence before deployment.
*   Implemented client-side privacy architecture, ensuring API tokens remain purely in transient browser memory.

### **SmartExpenseML - Client-Side NLP Expense Classifier**
*Stack:* JavaScript, Text Classification Engine, Regex Text Normalization | *Live:* https://sisigitadi.github.io/smartexpenseML | *Source:* https://github.com/sisigitadi
*   Built a browser-based NLP classifier parsing informal Indonesian transaction texts (e.g. 25rb, 5.5jt) with zero server data retention.
*   Implemented rule-based NLP text classification and regex token normalization executing entirely in-browser.

### **SCOPS - Security Operations Automation Platform**
*Stack:* Python, Streamlit, Wazuh SIEM REST API, SOP Incident Playbooks | *Live:* https://sisigitadi.github.io/scops | *Source:* https://github.com/sisigitadi
*   Engineered a centralized SOC triage dashboard that digests raw telemetry from Wazuh SIEM agents and classifies incidents into actionable High/Medium/Low priority queues.
*   Automated Tier-1 alert triage and incident response playbooks, reducing simulated MTTR by **45%** and MTTA by **55%** across 500+ daily telemetry events.

### **A.R.Y.A. - SOC Analytics & Threat Hunting Dashboard**
*Stack:* Python, Streamlit, Pandas, REST APIs, JSON/XML/CSV Parsers | *Live:* https://arya-soc.streamlit.app | *Source:* https://github.com/sisigitadi
*   Designed an automated telemetry ingestion pipeline that normalizes heterogeneous log streams (JSON, XML, CSV) into real-time threat hunting dashboards.
*   Streamlined log ingestion and threat detection workflows, cutting manual log review time by **30%** for Tier-1 analysts.

---

## PROFESSIONAL EXPERIENCE

### **Web Administrator (Contract)** | Ministry of Environment
*Mar 2026 - Present | Jakarta, Indonesia*
*   Maintained 5+ containerized web services on Linux Server/WSL with Wazuh SIEM monitoring and Linux hardening.
*   Streamlined telemetry analysis, reducing weekly manual log review time by **30%**.

### **Senior Programmer (Short-Term Contract)** | Ministry of Home Affairs
*Oct 2025 - Dec 2025 | Jakarta, Indonesia*
*   Developed backend components and 10+ REST API endpoints for a digital governance system processing 5,000+ daily records with SQL database optimization and secure data validation.
*   Resolved critical data ingestion bottlenecks, maintaining 99.5% API reliability across high-volume transaction cycles.

### **SOC Analyst (Part-Time)** | PT. Prospera Consulting Engineers
*Jan 2025 - Sep 2025 | Jakarta, Indonesia*
*   Developed 10+ Incident Response playbooks (aligned with NIST SP 800-61) for Tier-1 analysts with structured threat containment and escalation pathways.
*   Conducted Wazuh-based security baseline reviews and continuous vulnerability monitoring.
*   Reduced MTTA (Mean Time to Acknowledge) by **55%** across 500+ daily alerts after playbook rollout.

### **Incident Handling Operational (Short-Term Contract)** | Ministry of Home Affairs
*Oct 2024 - Dec 2024 | Jakarta, Indonesia*
*   Shift operations and real-time threat monitoring with Wazuh SIEM and FortiWeb WAF - triaging 500+ security alerts a day.
*   Restructured escalation pathways to maintain an average incident response time under 15 minutes from alert detection to containment.

### **AI Trainer & LLM Evaluator (Freelance)** | Multiple AI Evaluation Platforms
*Jun 2024 - Present | Remote*
*   Human-in-the-loop evaluation of model factuality, instruction-following, and safety; **1,000+ prompt-response pairs** reviewed and scored against platform guidelines.
*   Tested prompt stability and formatting adherence across 5+ major LLM model families to mitigate drift and hallucination.

### **IT & Operations Manager (Full-Time)** | ACE Ltd.
*May 2023 - Aug 2024 | Indonesia*
*   Maintained 99%+ IT infrastructure uptime and network operations for a 50+ person engineering team.
*   Built automated SQL database reporting scripts, cutting manual data entry time by **40%** for weekly operations.

### **Project Office Manager (Contract)** | Ministry of Public Works and Housing
*Dec 2020 - Apr 2023 | Indonesia*
*   Coordinated IT support, network connectivity, and field equipment across 4 concurrent infrastructure project sites, supporting 30+ field personnel.
*   Unified operational reporting across all 4 sites, reducing weekly data consolidation delays by **35%**.

### **Assistant to Office Manager (Short-Term Contract)** | Nippon Koei Co., Ltd.
*Jun 2020 - Nov 2020 | Indonesia*
*   Audited weekly budget variances and workflow tracking across 3 regional project teams with 100% on-time milestone reporting.
*   Eliminated administrative bottlenecks to ensure zero milestone delivery delays for the irrigation modernization project.

### **IT Manager (Full-Time)** | PT. Dipta Safari Jaya
*Jun 2014 - Apr 2020 | Indonesia*
*   Managed on-premise server fleet (3 core servers) and 40+ workstations over a 6-year tenure, maintaining 99%+ network availability.
*   Implemented scheduled daily backup protocols, achieving 0 data-loss incidents throughout the entire 6-year operational period.

### **IT Support Engineer & Hardware Technician (Full-Time)** | PT. Laju Karunia Jaya & Arya Mobile
*2002 - 2014 | Indonesia*
*   Managed technical support and infrastructure maintenance across 2 organizations over a 12-year foundation period, resolving 1,000+ hardware, OS, and network troubleshooting tickets.
*   Performed LAN cabling, PC assembly, server maintenance, hardware diagnostics, and end-user technical support.

---

## EDUCATION

*   **Bachelor of Computer Science (S.Kom.)** - Universitas Budi Luhur, Jakarta, Indonesia

---

## LANGUAGES

*   **Indonesian:** Native
*   **English:** Professional Working Proficiency

---

## CERTIFICATIONS

### AI & Automation
*   **Microsoft Azure AI Fundamentals (AI-900)** - Microsoft - 2025
*   **LLM-Based Tools & Gemini API Integration for Data Scientists** - Hacktiv8 - 2025
*   **Business Automation with n8n** - Kodeka Labs - 2025

### Cybersecurity, SecOps & Infrastructure
*   **Ubuntu Linux Professional Certification** - Canonical - 2025
*   **Penetration Testing Professional** - Cybrary - 2025
*   **Certified Ethical Hacker Foundation** - LinkedIn Learning - 2025
*   **Indonesia Cross-Sectoral Cyber Exercise #9** - BSSN - 2024
*   **Certified SOC Analyst** - Cyber Academy Indonesia - 2024
*   **DevSecOps for Software Security & IT Auditor** - Kelas.work - 2024
