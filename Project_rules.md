...
9. **WORKER VISITOR — SECRETS & GUARD INVARIANTS (v2.6.2)**: `DASHBOARD_KEY` dan `IP_HASH_SALT` adalah secret Cloudflare Worker (`worker-visitor/`), di-set **hanya** via `npx wrangler secret put` (pakai `printf '%s'` tanpa trailing newline — `echo` menambah newline dan membuat perbandingan constant-time `safeEqual` menolak key). **Dilarang memakai blok `[vars]` di `wrangler.toml` untuk keduanya** — nilai `[vars]` bernama sama **menimpa secret** saat deploy (terkonfirmasi di deploy pertama). Invariant keamanan worker yang wajib dipertahankan: (a) **fail-closed `IP_HASH_SALT`** — `recordVisit()` menolak merekam kunjungan (HTTP 500, tidak ada insert) jika salt missing/placeholder `CHANGE_ME_*`; dilarang mengembalikan fallback salt konstan seperti `'salt'` (hash menjadi bisa ditebak & identik lintas deploy, mengalahkan anonimisasi UU PDP); (b) **body guard `POST /hit` 10 KB** — `readJsonBody()` menolak payload lebih besar (HTTP 413 `payload_too_large`) via `Content-Length` maupun stream chunked; (c) **`Referrer-Policy: no-referrer`** pada semua respons HTML dashboard/login (URL-nya memuat `?key=…`). Setiap perubahan `worker-visitor/worker.js` WAJIB lolos `node --test worker-visitor/worker.test.js` (26 route test — gerbang CI preflight ketiga bersama audit 13 & pytest 62).
