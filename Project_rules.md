...
4. **SINGLE SEO CANONICAL SOURCE OF TRUTH (porto-first, v2.11.1)**: `rel="canonical"`, `og:url`, `twitter:url`, Schema.org JSON-LD `url`, `robots.txt` (`Sitemap:`), and `sitemap.xml` (`<loc>`) MUST all reference the identical canonical domain `https://porto.sigitadi.id` (the GitHub Pages subdomain, `CNAME`, serving this content). The apex `https://sigitadi.id` hosts a separate Next.js app (MyWebPorto) — never point this repo's canonical set at URLs whose content differs. Do not reintroduce `sisigitadi.github.io/portofolio`, `sigitadi.my.id`, the `github.com/sisigitadi/portofolio` repository page, or any divergent URL across these files.
